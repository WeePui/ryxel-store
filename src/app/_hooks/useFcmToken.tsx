...
import { registerFcmToken } from "@libs/apiServices";
...
// Helper function to get user's JWT token from cookies
async function getUserToken(): Promise<{ value: string } | null> {
  try {
...
const useFcmToken = () => {
  ...
  const loadToken = useCallback(async () => {
    ...
    // Step 7.1: Save FCM token to user record
    try {
      const userToken = await getUserToken();
      if (userToken) await registerFcmToken(token, userToken);
    } catch (error) {
      console.error("Failed to register FCM token:", error);
      // Don't show error to user since this is not critical for the main flow
    }

    isLoading.current = false;
  }, [setToken, setNotificationPermissionStatus]);
