...
function Modal({ children, onClose, showCloseButton = true }) {
  ...
  return createPortal(
    <div className="fixed left-0 top-0 z-[100] h-screen w-full bg-grey-400 bg-opacity-30 backdrop-blur-sm transition-all duration-500">
      <div
        className="fixed left-1/2 top-1/2 max-h-[90vh] max-w-full -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl bg-white px-12 py-16 shadow-lg transition-all duration-500"
        ref={ref}
      >
        {showCloseButton && (
          <button
